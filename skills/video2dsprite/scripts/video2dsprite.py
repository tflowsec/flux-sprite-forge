#!/usr/bin/env python3
"""Postprocess FLUX 3 image-to-video clips into dense 2D sprites.
...
The video itself comes from the FLUX 3 API (scripts/flux_video.py, i2v mode).
This script only needs ffmpeg, Pillow, and numpy.
"""
...
def write_readme(out_dir: Path, meta: dict) -> None:
    lines = [
        "Video2dsprite output (FLUX 3 i2v pipeline)",
        "==========================================",
        "base/           base still on #FF00FF",
        "video/          FLUX 3 image-to-video clip (+ .job.json task record)",
        "frames-raw/     decoded frames",
        "frames-clean/   chroma-keyed RGBA frames",
        "sprite/         sampled normalized sprites + strips/grids/GIFs",
        "pipeline-meta.json",
        "",
        "The clip was generated with the FLUX 3 video API (scripts/flux_video.py).",
        "To re-sample denser frame sets without a new video run:",
        "python video2dsprite.py sample --clean-dir ...",
        "",
...
def cmd_process(args: argparse.Namespace) -> int:
    ...
    meta = {
        "skill": "video2dsprite",
        "platform": "FLUX 3 video API (flux_video.py i2v for the generation step)",
        "name": args.name,
