...
If matching a project sprite: pass the existing frame with `--input-image` and only change pose/background to magenta if needed. Prefer compositing a known good frame onto magenta in code when the art already exists.

## Video (FLUX 3 `i2v` via `flux_video.py`)
