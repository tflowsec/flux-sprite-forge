...
- raw sprite art must come from the FLUX API (`flux_generate.py`), not Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots
...
- Pass the reference image to `flux_generate.py` with `--input-image`. Local files are base64-encoded automatically; do not assume the model can see anything that was not passed as an input image.
- In the prompt, say `use image 1 (the input image) as the visual reference`.
...
When using a layout guide, pass the guide image with `--input-image` and write:

```text
Use the layout guide image (the input image) as a layout-only reference. Use it only to understand the rows, columns, equal invisible frame slots, centering, spacing, and safe padding. Do not reproduce the guide: no visible boxes, no safe-area rectangles, no center marks, no labels, no borders, no guide background.
```
...
3. Pass both images as input images (`--input-image <master> --input-image <anchor>`):
   - Image 1, master frame: identity, art style, anatomy, camera, and material reference
   - Image 2, anchor sheet: slot, scale, body-root, feet-line, and padding reference
