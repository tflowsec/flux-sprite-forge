...
description: "Generate and postprocess general 2D game assets and animation sheets: pixel-art sprites, clean HD map props, creatures, characters, NPCs, spells, projectiles, impacts, props, summons, and transparent GIF exports. Use when the agent should infer the asset plan from a natural-language request, match a reference or map art style, generate solid-magenta raw sheets with the FLUX API through scripts/flux_generate.py, and use the local processor only for chroma-key cleanup, frame extraction, alignment, QC, and transparent exports. Requires BFL_API_KEY."
---
...
- Generate every raw image with the FLUX API through `scripts/flux_generate.py` (submit, poll, download in one call).
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from the FLUX API.
- When the user provides or implies a visual reference, pass the actual image file to the API as an input image: `--input-image <path>` (repeatable, up to 8). Local files are base64-encoded by the script automatically. Reference images by number in the prompt, e.g. "the character from image 1". Never describe a reference from memory instead of passing the file.
...
- For high-value grounded player/hero body actions, prefer a character anchor sheet when consistent scale or feet placement matters: repeat one accepted master frame at the intended size and feet line in every cell, then use that sheet as a scale/root template while asking the FLUX API (anchor sheet as input image) to change only the poses. Do not use a grounded anchor sheet for jumps, knockback, airborne motion, projectiles, or FX.
...
- Pass the reference as an input image. Freshly generated references are already local files; pass their saved path directly with `--input-image`.
...
- Do not ask the FLUX API to generate unrelated action rows in one raw sheet, such as `row 1 idle, row 2 run, row 3 shoot, row 4 jump`, for a controllable hero or main character.
- Do not ask the FLUX API to generate raw single-row action strips such as `1x4 idle`, `1x4 run`, `1x4 shoot`, or `1x4 jump` for a controllable hero, character, creature, NPC, enemy, summon, or animated prop.
...
If a layout guide is useful, generate one before calling the FLUX API:
...
Then pass the guide as an input image (`--input-image <guide.png>`) and tell the FLUX API to use it only for invisible slot count, spacing, centering, and safe padding. The output must not reproduce guide boxes, safe-area rectangles, center marks, labels, borders, or guide background.
...
Pass both the master frame and the anchor sheet as input images (`--input-image <master.png> --input-image <anchor.png>`). State that Image 1 (the master) locks identity/style and Image 2 (the anchor sheet) locks slot positions, camera distance, standing-equivalent scale, body root, feet line, and padding. Ask the model to change only the action poses and never reproduce guides, borders, labels, or separators.
...
- `flux-2-klein-9b` — cheap and fast; default for iteration and QC loops
- `flux-2-pro` — default for accepted/final art
- `flux-2-max` — highest quality for hero and main-character art
- `flux-2-flex` — when precise in-image text or unusual aspect handling matters

Reference-guided generation passes input images (repeat `--input-image`, up to 8; the prompt must reference them by number: "Image 1 ... Image 2 ..."):

```bash
python scripts/flux_generate.py \
  --prompt-file <run-dir>/prompt.txt \
  --input-image <run-dir>/references/master-frame.png \
  --input-image <run-dir>/references/attack-anchor-2x3.png \
  --output <run-dir>/raw-sheet.png
```

All FLUX.2 models support image editing natively through `input_image` — there is no separate edit endpoint. Multi-reference composition, style transfer, and identity preservation all use the same flags.
...
After generation, the raw PNG is exactly where `--output` pointed. Keep it in place; the processor works from that path.
...
- `scripts/flux_generate.py`: FLUX image API transport (submit, poll, download, input-image references)
