---
name: generate2dmap
description: "Generate and revise production-oriented 2D game maps with the FLUX image API as the default visual asset source, choosing a visual model, runtime object model, collision model, art direction, and engine/export target. Use when the agent needs to create or integrate RPG maps, monster-taming maps, tactical arenas, battle backgrounds, side-scroller/parallax scenes, tilemaps, layered raster maps, clean HD hand-painted maps, pixel-inspired maps, prop packs, collision zones, walkable areas, or map previews. Requires BFL_API_KEY."
---
...
This skill is image-generation-first for visual assets. Use the FLUX image API through `scripts/flux_generate.py` as the default creative art source for base maps, in-world reference mockups, dressed references, stage references, prop sheets, prop sprites, tileset art, parallax layers, battle backgrounds, and other visible map assets.

The agent must write the creative image prompts itself. Do not use scripts to generate creative prompts or to procedurally draw final visual art. Scripts may assemble, slice, chroma-key, crop, validate, compose previews, emit JSON metadata, call the FLUX API as a dumb transport (`flux_generate.py`), and wire image-generated assets into engine-native files such as Godot `.tscn` scenes.
...
When generating an in-world reference mockup from an existing generated base/background, the prior image must be treated as an active visual reference, not just a loose style hint:

1. Save the base/background image first.
2. Pass that exact image to the next `flux_generate.py` call with `--input-image <saved-file>`. The script base64-encodes local files automatically.
3. In the prompt, explicitly say to use image 1 (the input image) as the visual reference.
4. Describe concrete features from the referenced image that must be preserved, such as camera framing, horizon, road or water shapes, terrain boundaries, entrance/exit direction, major silhouettes, empty pads, and landmark positions.
...
- `visual_asset_source`: flux_api | existing_assets | procedural_placeholder
...
- Use the FLUX API (`flux_generate.py`) as the visual asset source.
...
2. Choose the pipeline axes.
   ...
   - Select `visual_asset_source`. Default to `flux_api`; use `existing_assets` only when the project already has suitable art; use `procedural_placeholder` only when explicitly requested.
   - Treat `hybrid` as a result of combining axes, not as a primary category.

3. Produce assets.
   - Write the creative prompts manually and generate visible map art with `scripts/flux_generate.py` unless the user explicitly chose existing assets or procedural placeholders.
   - For baked raster maps, generate one background with `scripts/flux_generate.py`, or edit/use an existing image as `--input-image` when supplied, then add optional collision/zones metadata.
...
2. Pass the base image to `flux_generate.py` as `--input-image`; do not rely on a verbal description of the base as the reference.
...
Use `scripts/extract_prop_pack.py` after generating a solid-magenta prop sheet. The extractor chroma-keys the magenta internally; if the sheet has antialiased magenta fringe, tighten its `--threshold` / `--edge-threshold` / `--edge-clean-depth` settings instead of adding a separate cleanup pass. Use `scripts/compose_layered_preview.py` to verify placement over the base map.
...
2. Pass both images as input images (`--input-image <base> --input-image <reference>`) before writing object lists or object/prop image prompts. Do not rely on memory of the images alone.
...
4. For each visible runtime object, choose exactly one asset strategy:
   - generate a separate transparent asset with `$generate2dsprite` or direct `flux_generate.py`
...
2. Pass the background to `flux_generate.py` as `--input-image`; do not rely on a verbal description of the background as the reference.
