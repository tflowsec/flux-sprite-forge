...
The extractor chroma-keys the magenta background internally (`--threshold` for the flood-fill key, `--edge-threshold` for antialiased edge pixels, `--edge-clean-depth` for fringe erosion). When the sheet has heavy antialiased magenta fringe or the props will be composited over a dark/detailed base, tighten those settings instead of writing a separate cleanup pass.
